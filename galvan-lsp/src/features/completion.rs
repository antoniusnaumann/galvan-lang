...
//!
//! Offers top-level functions and types declared in the current document plus a
...
pub fn completion(document: &Document) -> Vec<CompletionItem> {
    ...
    if let Some(segmented) = document.segmented.as_ref() {
        for func in &segmented.functions {
            let detail = span_text(text, func.signature.span).to_string();
        ...
        for ty in &segmented.types {
            let span = type_decl_span(ty);
            let header = span_text(text, span);
