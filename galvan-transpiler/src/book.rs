...
use galvan_transpiler::{transpile_sources_with_rustdoc_warnings, TranspileOutput};
...
fn run() -> Result<(), String> {
    ...
    for path in files {
        ...
        for captures in pattern.captures_iter(&original) {
            ...
            let actual = match transpile_example(galvan) {
                Ok(actual) => actual,
                Err(error) => {
...
fn transpile_example(source: &str) -> Result<String, String> {
    let mut outputs = transpile_sources_with_rustdoc_warnings(
