...
impl Transpile for ConstructorCall {
    fn transpile(&self, ctx: &Context, scope: &mut Scope) -> String {
                ...
                self.arguments
                    ...
                    .map(|arg| {
                        ...
                        if let Some(expected_type) = field_types.get(arg.ident.as_str()) {
                            ...
                            let cast_expr = cast(&arg.expression, expected_type, Ownership::UniqueOwned, ctx, scope);
                            format!("{}: {}", arg.ident.transpile(ctx, scope), cast_expr)
                        } else {
                            // Fallback to original behavior if field not found
                            arg.transpile(ctx, scope)
                        }
                    })
                    .collect::<Vec<_>>()
                    .join(", ")
        ...
        format!("{} {{ {} }}", self.identifier.transpile(ctx, scope), arguments_str)
    }
}
