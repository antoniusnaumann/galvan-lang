...
use crate::{Transpile, TranspilerError};
use galvan_ast::{
    ConstructorCall, ConstructorCallArg, ExpressionKind, InfixOperation, MemberOperator, Ownership,
    TypeDecl,
};
...
impl Transpile for InfixOperation<MemberOperator> {
    fn transpile(&self, ctx: &Context, scope: &mut Scope, errors: &mut ErrorCollector) -> String {
        ...
        match operator {
            ...
            MemberOperator::SafeCall => match lhs.infer_owned(ctx, scope, errors) {
                Ownership::SharedOwned | Ownership::MutBorrowed => {
                    transpile!(
                        ctx,
                        scope,
                        errors,
                        "{}.as_ref().map(|__elem__| {{ __elem__.{}.clone() }})",
                        lhs,
                        rhs
                    )
                }
                ...
                Ownership::Borrowed => {
                    transpile!(
                        ...
                        scope,
                        errors,
                        "{}.map(|__elem__| {{ __elem__.{}.clone() }})",
                        lhs,
                        rhs
                    )
                }
                Ownership::Ref => unimplemented!(),
            },
        }
    }
}
    ...
    fn transpile(&self, ctx: &Context, scope: &mut Scope, errors: &mut ErrorCollector) -> String {
        ...
        let arguments_str = if let Some(type_item) = type_decl {
            if let TypeDecl::Struct(struct_decl) = &type_item.item {
                ...
                let provided_args: std::collections::HashMap<_, _> = self
                    .arguments
                    .iter()
                    ...
                    if let Some(arg) = provided_args.get(field_name) {
                        ...
                        if let Some(expected_type) = field_types.get(field_name) {
                            ...
                            final_args.push(format!(
                                "{}: {}",
                                arg.ident.transpile(ctx, scope, errors),
                                cast_expr
                            ));
                        } else {
                        ...
                        final_args.push(format!(
                            "{}: {}",
                            member.ident.transpile(ctx, scope, errors),
                            default_transpiled
                        ));
                    } else {
        ...
        format!(
            "{} {{ {} }}",
            self.identifier.transpile(ctx, scope, errors),
            arguments_str
        )
    }
}
